...
$('button#faqListFeedBtnN').click(function(){
    $('div.faqListFeedBack').html('<span>도움이 되지 않은 사유를 알려주세요.<br/>더 나은 도움말 제공을 위해 소중히 활용하겠습니다.</span>')
        ...
        .append($('<button id=faqListFeedBtnNSubmit type="button" style="width: 135px; padding: 8px 0; font-size: 16px; line-height: 26px; letter-spacing: -.47px; border: 1px solid #f0f0f0; border-radius: 22px; color: #222222;">평가 완료</button>'));
});
...
$(document).on('mouseenter', 'button#faqListFeedBtnNSubmit', function() {
    ...
    var inputVal = $('div.faqListFeedBack textarea').val();
    if (inputVal.trim() == '') {
        alert('소중한 의견을 남겨주세요.');
        return;
    }
    $('div.faqListFeedBack').html('<img src="https://ssl.pstatic.net/static/help/new_help/img/svg/feedback_submit.svg" width="24" height="24"><span>&nbsp;&nbsp;소중한 의견에 감사합니다.<span>');
});
